...
.PARAMETER OutputPath
    Optional. Base output path. Defaults to "D:\intoon in progress\Appgetter"
.PARAMETER IconPath
...
param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$OutputPath = "D:\intoon in progress\Appgetter",
...
# Diagnostic logging
$script:DiagnosticLogDirectory = Join-Path ([System.IO.Path]::GetTempPath()) "Appgetter"
$script:DiagnosticLogPath = Join-Path $script:DiagnosticLogDirectory ("appgetter-{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss"))
...
function Get-WingetIdFromDialog {
    ...
    $title = "Appgetter - Enter Winget Package ID"
...
function Select-WingetPackage {
    ...
    $title = "Appgetter - Select Package"
...
$win32LobAppJson = @{
    ...
    notes = "Generated by Appgetter at $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') [Winget|$packageId]"
    publisher = $publisher
