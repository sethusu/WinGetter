...
| **Licensing details** | $(if ([string]::IsNullOrWhiteSpace($LicensingInfo)) { 'Not provided' } else { $LicensingInfo }) |
...
"@
    ...
    $appJson = @{
        ...
        licensingDetails = $LicensingInfo
    }
    ...
    $win32LobAppJson = @{
        ...
        notes = "Generated by Wingetter at $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') [Winget|$($PackageDetails.PackageId)]$(if (-not [string]::IsNullOrWhiteSpace($LicensingInfo)) { " [License|$LicensingInfo]" } else { '' })"
        publisher = $PackageDetails.Publisher
