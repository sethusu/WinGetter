...
function Normalize-WingetPackageId {
    <#
    .SYNOPSIS
        Normalizes winget package IDs that contain stray whitespace (for example "Valve. Steam").
    #>
    [CmdletBinding()]
    param(
...
function ConvertFrom-WingetShowOutput {
    ...
    foreach ($line in $lines) {
        if ($line -match '^\s*Found\s+(.+?)\s+\[(.+?)\]\s*$') {
            ...
            $package.Id = Normalize-WingetPackageId -Id $matches[2].Trim()
            continue
        }
...
function ConvertFrom-WingetSearchOutput {
  ...
  foreach ($line in $lines) {
    ...
    for ($i = $headerIndex + 1; $i -lt $lines.Count; $i++) {
        ...
        $name = $row['Name']
        $rawId = $row['Id']
        $truncatedId = $rawId -match '…|\.\.\.$'
        $id = Normalize-WingetPackageId -Id ($rawId -replace '…|\.\.\.$', '')
