...
function Get-LogoFromWeb {
    ...
    # Try common CDN/hosting patterns when org/repo are known
...
function Start-WingetDownloadWithProgress {
    ...
    $job = Start-Job -ScriptBlock {
        ...
        $args = @('download', $pkgId, '--exact', '--download-directory', $dir, '--disable-interactivity', '--accept-source-agreements')
        if ($supportsPkgAgreements) {
            $args += '--accept-package-agreements'
        }
        if ($pkgVersion) {
            $args += @('--version', $pkgVersion)
        }
        winget @args 2>&1
    ...
    if ($jobOutput) {
        $successIndicators = $jobOutput | Select-String -Pattern '(?i)(Successfully downloaded|Installer hash verified|Download complete)' -Quiet
        $errorIndicators = $jobOutput | Select-String -Pattern '(?i)(failed when|download failed|no applicable installer|error 0x|An unexpected error|Unable to download)' -Quiet
        if ($errorIndicators -and -not $successIndicators) {
...
try {
    ...
    if ($Version -and $foundVersion -ne $Version) {
        Write-StatusError "Requested version $Version does not match found version $foundVersion"
        $foundVersion = $Version
    }
    ...
    Write-StatusError "Failed to search Winget: $_"
    exit 1
}
...
try {
    ...
    Write-StatusError "Failed to download installer: $_"
    exit 1
}
...
try {
    ...
    Write-StatusError "Failed to calculate hash: $_"
    $installerHash = ""
}
        ...
        if (`$allKeys) {
            ...
            if (`$uninstallKeys) {
                foreach (`$key in `$uninstallKeys) {
                    ...
                    if (`$nameMatch) {
                        # Some vendors (for example JetBrains) store marketing versions in DisplayName
                        # and build numbers in DisplayVersion. Prefer a version embedded in DisplayName.
                        `$extractedVersion = `$null
                        if (`$key.DisplayName -match '(\d+(?:\.\d+){2,3})') {
...
try {
    ...
    Write-StatusError "Failed to create IntuneWin package: $_"
...
$intunewinSummary = if ($intunewinFile) { $intunewinFile } else { '(not created)' }
Write-Host @"
...
- IntuneWin Package: $intunewinSummary
