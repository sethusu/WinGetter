...
try {
    ...
    $packageId = Normalize-WingetPackageId -Id $selectedPackage.Id
    ...
    if ($showPackage.Id) {
        $packageId = Normalize-WingetPackageId -Id $showPackage.Id
    }
    ...
    if ($Version -and $foundVersion -ne $Version) {
        Write-StepError "Requested version $Version does not match found version $foundVersion"
        $foundVersion = $Version
    }
    ...
    Write-StepError "Failed to search Winget: $_"
    exit 1
}
...
try {
    ...
    Write-StepError "Failed to download installer: $_"
    exit 1
}
...
try {
    ...
    Write-StepError "Failed to calculate hash: $_"
    $installerHash = ""
}
        ...
        if (`$allKeys) {
            ...
            if (`$uninstallKeys) {
                foreach (`$key in `$uninstallKeys) {
                    ...
                    if (`$nameMatch -and `$key.DisplayName) {
                        # Products like JetBrains store marketing versions in DisplayName
                        # (e.g. "WebStorm 2025.3.1.1") while DisplayVersion may be a build number.
...
try {
    ...
    Write-StepError "Failed to create IntuneWin package: $_"
...
$intunewinSummaryLine = if ($intunewinCreated) {
    ...
    "- IntuneWin Package: (not created - see errors above)"
}
$intunewinFileSummary = if ($intunewinCreated) {
    "- $($installerFile.BaseName).intunewin (Intune package)"
} else {
    "- $($installerFile.BaseName).intunewin (not created)"
}
