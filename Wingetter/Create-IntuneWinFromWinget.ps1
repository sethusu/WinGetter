...
. (Join-Path $scriptRoot "WingetSearchParser.ps1")
...
try {
    ...
    $supportsDisableInteractivity = winget search --help 2>&1 | Select-String -Pattern "disable-interactivity" -Quiet

    $searchAttempts = New-Object System.Collections.Generic.List[object]
    $appLooksLikeId = $AppName -match '^[A-Za-z0-9][A-Za-z0-9._-]*\.[A-Za-z0-9._-]+$'

    if ($appLooksLikeId) {
        $searchAttempts.Add([PSCustomObject]@{
            Label = "exact-id"
            Args = @("--id", $AppName, "--exact")
        })
    }
    ...
    foreach ($attempt in $searchAttempts) {
        $commandArgs = @("search") + $attempt.Args + @("--accept-source-agreements")
        if ($supportsPackageAgreements) {
            $commandArgs += "--accept-package-agreements"
        }
        if ($supportsDisableInteractivity) {
            $commandArgs += "--disable-interactivity"
        }

        Write-Host "Trying winget search strategy '$($attempt.Label)'..." -ForegroundColor DarkCyan
        $searchResult = & winget @commandArgs 2>&1
        $searchExitCode = $LASTEXITCODE

        $parsedPackages = Get-WingetPackagesFromSearchOutput -SearchOutput $searchResult
        if ($parsedPackages.Count -gt 0) {
            Write-Host $searchResult
            $selectedPackage = Select-WingetPackage -SearchOutput $searchResult
            break
        }

        $noMatch = $searchResult | Select-String -Pattern "No package found matching input criteria\.?" -Quiet
        if ($searchExitCode -ne 0 -and -not $noMatch) {
            Write-Host "Winget search output from '$($attempt.Label)':" -ForegroundColor Yellow
            Write-Host $searchResult
            throw "Winget search failed with exit code $searchExitCode for strategy '$($attempt.Label)'."
        }
    }
